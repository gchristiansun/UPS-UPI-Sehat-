...
# Mengubah file txt menjadi data list
    ...
    if os.path.exists(file_txt):
        ...
        for line in lines:
            ...
            globalVar.list_data.append(newLine)

        for item in globalVar.list_data:
            break
 
# Input nama user yang terdaftar saat registrasi      
    nama_akun = input("Nama Akun: ")
    while True:
        if nama_akun == globalVar.list_data[0][1].strip():
            break
...
# Input password yang terdaftar saat registrasi
    ...
    while True:
        if login_password == globalVar.list_data[2][1].strip():
